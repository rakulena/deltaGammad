...
{
    RooRealVar& x = (RooRealVar &)parent.convVar(); // binboundaries not const...
    //RooRealVar& x = parent.convVar(); // For ROOT v6.18
    ...
    const double rxmin = parent.convVar().getMin(rn);
    const double rxmax = parent.convVar().getMax(rn);
    ...
    _x = dynamic_cast<RooRealVar*>(x.clone((std::string(x.GetName()) + "_" +rn).c_str()));
    //RooRealVar *_x = (RooRealVar*)x.clone((std::string(x.GetName()) + "_" +rn).c_str());
	...
	Int_t code, const char* rangeName) const
{
	...
	// std::unique_ptr<RooArgSet> iset( new RooArgSet(
	//	    _cacheMgr.selectFromSet1(*vars, code - 1)));
