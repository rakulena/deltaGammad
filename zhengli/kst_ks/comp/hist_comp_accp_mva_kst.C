	...
	)
{
   ...
   string intree1 = Form("/lzufs/user/zhanghy/Sample/Train/Bd2JpsiKstar_mc_%i_BDT.root/DecayTree",year);
   
   char varname[] = "BDT";
   //string weight1 = "phsp(m_hh_true)";
   string weight2 = "1./exp(-1/1.519*Bd_ConstJpsi_tau)";
   string cut1 = "abs(Bd_ConstJpsi_M-5279.65)<30)";
   ...
   for( int i=0; i<n; i++ ){
      ...
      seltree->Project(h_time[i]->GetName(),"Bd_ConstJpsi_tau",selcut1.c_str());//*Bd_ConstJpsi_tau Bd_D0ToKstar_tau
   ...
   if( year==-1 ){
      ...
      Printpic( pic1, Form("output/%s_accp_mva_time_%i_kst",prefix.c_str(),year), 1 );
   }

}

void hist_comp_accp_mva_kst(
	){
