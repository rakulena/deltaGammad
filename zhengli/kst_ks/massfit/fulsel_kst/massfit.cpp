...
void massfit::Loop( Long64_t runevt, const char *runmode )
{
   ...
   string picname = Form("../output/Bdjpsikstar_%s%s%s",prefix.c_str(),endfix.c_str(),Year.c_str());
   string ofroot  = Form("/lzufs/user/zhanghy/Sample/afterBDT/after_Bd2JpsiKstar_data%s_sw%s.root",Year.c_str(),endfix.c_str());
   //Define fit pdfs
   //---------------------------------------------
