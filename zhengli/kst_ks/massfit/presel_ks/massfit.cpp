...
void massfit::Loop( Long64_t runevt, const char *runmode )
{
   ...
   string picname = Form("../output/presel_bdjpsikshort_%s%s%s",prefix.c_str(),endfix.c_str(),Year.c_str());
   string ofroot  = Form("/lzufs/user/zhanghy/Sample/data/Bd2JpsiKshort/presel_Bd2JpsiKshort_data%s_sw%s.root",Year.c_str(),endfix.c_str());
   //Define fit pdfs
   //---------------------------------------------
   ...
   //RooRealVar *sigma2 = new RooRealVar("sigma2","",10.,0.,20.);
   //RooRealVar  *frac = new RooRealVar("frac","",0.5,0.,1.);
   //RooGaussian *sig = new RooGaussian("sig","",*mass,*mean1,*sigma1);
   //RooGaussian *sig1 = new RooGaussian("sig2","",*mass,*mean1,*sigma2);
