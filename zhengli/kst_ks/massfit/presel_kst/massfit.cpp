...
void massfit::Loop( Long64_t runevt, const char *runmode )
{
   ...
   int    nBin = 100; 
   double mvacut = 0.09; // same as the signal channel
   double mass_l = 5220.;//
   double mass_h = 5330.;//
   ...
   string picname = Form("../output/presel_bdjpsikstar_%s%s%s",prefix.c_str(),endfix.c_str(),Year.c_str());
   string ofroot  = Form("/lzufs/user/zhanghy/Sample/data/Bd2JpsiKstar/presel_Bd2JpsiKstar_data%s_sw%s.root",Year.c_str(),endfix.c_str());
   //Define fit pdfs
   //---------------------------------------------
   ...
   RooRealVar *nsig1 = new RooRealVar("nsig1","", 33501,0,5e6);
   RooRealVar *nbkg1 = new RooRealVar("nbkg1","", 5000,0,1e7);
   ...
   if(!( Bd_ConstJpsi_M>mass_l && Bd_ConstJpsi_M<mass_h)) continue;
   ...
   if( Nmc ){
	...
	leg->AddEntry(frame->findObject("1"), "#it{B^{0}#rightarrowJ/#psiK^{+}#pi^{#minus}}","L");
   ...
   const char* XTitle = "#it{m}(#it{J/#psi}#it{K^{+}#pi^{#minus}}) [MeV/#it{c}^{2}]";
